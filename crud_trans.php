...
//	if ($tabla=="distribuidor" or $tabla=="bodega") {$sIndexColumn = "codigo";}
	...
	if ( isset($_GET['sSearch']) && $_GET['sSearch'] != "" )
	{
		...
		{
			if ($aColumns[$i]!='btnaccion') {$sWhere .= $aColumns[$i]." LIKE '%".mysql_real_escape_string( $_GET['sSearch'] )."%' OR ";}
		}
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if ( isset($_GET['bSearchable_'.$i]) && $_GET['bSearchable_'.$i] == "true" && $_GET['sSearch_'.$i] != '' )
		{
			...
			if ($aColumns[$i]!='btnaccion') {$sWhere .= $aColumns[$i]." LIKE '%".mysql_real_escape_string($_GET['sSearch_'.$i])."%' ";}
		}
	}
