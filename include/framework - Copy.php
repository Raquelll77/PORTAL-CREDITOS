...
define("app_version", "1.0", true);  // Version de Applicacion

define("app_combo_si_no", '<option value="N">NO</option><option value="S">SI</option>', true);
...
function sqladd($sql,$valor,$tipo) {
	$salida="";
	if ($valor=="" or is_null($valor)) {$salida="";} else {
...
function agregar_nonulo($textoantes,$texto,$textodespues) {
	$salida="";
	if ($texto=="" or is_null($texto)) {$salida="";} else {
...
function fechademysql($fecha){ //  dd/mm/yyyy
	//return date('Y-m-d', strtotime(str_replace('-', '/', $fecha)));
...
function fechademysql_mini($fecha){ //  dd/mm/yy
	//return date('Y-m-d', strtotime(str_replace('-', '/', $fecha)));
...
function horademysql($fechahora,$solo_fecha=false){ //  hh:mm am/pm dd/mm/yyyy
	
	$formato="h:i a";
	if ($solo_fecha==true) {$formato="d/m/y";	}
...
function fechahora_mysql($fechahora){ //  hh:mm am/pm dd/mm/yyyy
...
function leercheckbox($valor){
	...
	if ($valor=='S' ) { $salida= " checked";} else {$salida= "";}
...
function enviar_correo($email_a,$subject,$cuerpo_html,$cuerpo_sinhtml){
	$salida=false;
		...
		$salida=true;
		if(!$mail->Send()) {	$salida=false;	  // echo 'Mailer Error: ' . $mail->ErrorInfo;
			}
...
function campo($nombre,$etiqueta,$tipo,$valor,$adicional,$valor2="",$corto="",$columna1=3,$columna2=9) {
$salida="";
 ...
 $salida="";
  ...
  {  //upload
   ...
   if ($valor=="" or $mostrar_upload=="SI")  {
         ...
         $salida.=' 
      &nbsp;&nbsp;&nbsp;    
  <a href="#" class="btn btn-default" onclick="return false;" data-toggle="collapse" data-target="#UPL'.$n.'"><span class="glyphicon glyphicon-cloud-upload" aria-hidden="true"></span></a>
...
//   'use strict';
          $salida .= "<script>        
                    $(function () {
                       ...
                       if ($id_solicitud<>"") { // para guardar documento de un solo
                           $salida .= "                           
                            var respuesta2=  $.ajax({
                                type: \"GET\",
                            ...
                            if (respuesta2.trim()=='OK') {
                                ...
                                $('#progress_$nombre').hide();
                                 ...
                                 } else {showmessage('Error',respuesta2.trim())}
...
function boton_verificar($n,$cid,$comentario=false,$usuario="",$asignados,$campo_unico) {
   ...
   if ($estado_asignado<>"") {
       ...
       if ($estado_asignado=="Confirmado") {$salida='<span class="glyphicon glyphicon-ok" aria-hidden="true"></span>';
       ...
       if ($estado_asignado=="Vendedor") {$salida='<span class="glyphicon glyphicon-envelope" aria-hidden="true"></span> Gestion pendiente';
                                          $salida.=$asignados["desc2"][$n]; 
                                           if (tiene_permiso(18) and $campo_unico=="") {  $salida.=" <br> <a  href=\"#\" class=\"btn btn-default btn-sm\" onclick=\"actualizarbox('pagina','creditos_gestion.php?a=5b1".$asignados["btn"][$n]."') ; return false;\" >Responder</a>";}
                                            }
...
function verificar_password($vpass, $vpass2) {
	...
	if (trim($hash1) === trim($hash2)) {$salida=true;}
	return $salida;
}
...
function valores_combobox_db_distribuidores_bodegas_asignados($id,$tabla){
 ...
 function get_fecha_sistema() {  //formato de salida= mysql yyyy-mm-dd
...
function get_mes($fecha) { // formato entrada yyyy-mm-dd
if ($fecha<>""){
...
function get_dia($fecha) { // formato entrada yyyy-mm-dd
if ($fecha<>""){
...
function get_anio($fecha) { // formato entrada yyyy-mm-dd
if ($fecha<>""){
...
function get_spanish_day($fecha) { // formato entrada yyyy-mm-dd
...
function get_spanish_date_mmm($fecha) {// formato entrada dd-mm-yyyy Salida dd-mmm-yyyy

	$pieces = explode("-", $fecha);
	$mes = intval($pieces[1]);
	$month = array("", "Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic");
	if (isset($month[$mes])) { $salida = $pieces[0] . "-" . $month[$mes] . "-" . $pieces[2];
	} else {$salida = $fecha;
	}
	return $salida;
}
...
function insertar_sql($sql){
	...
	if (!mysqli_connect_errno()) {	  
		...
		if ($conn->query($sql) === TRUE) { 	$salida = mysqli_insert_id($conn); }
...
function ejecutar_sql($sql){
	...
	if (!mysqli_connect_errno()) {	  
		...
		if ($conn->query($sql) === TRUE) { 	$salida = true; }
...
function get_gestiones_sql() // genera el sql para mostrar gestiones segun permisos de usuario
{
    ...
    if (tiene_permiso(19)) {$salida.=" and prestamo_gestion.gestion_estado='Creditos'";} 
    else {
         ...
         if (tiene_permiso(22)) {
                 ...
                 if (tiene_permiso(7)){
                    ...
                    if ($_SESSION['usuario_bodega']<>"") {$sql=" and  prestamo_gestion.bodega='".$_SESSION['usuario_bodega']."'";}                
                    }
                    
                     $salida.=" and prestamo_gestion.gestion_estado='Vendedor' $sql";
         } else {$salida.=" and prestamo_gestion.usuario_dirigido='".$_SESSION['usuario']."' and prestamo_gestion.gestion_estado='Vendedor'";}
...
function enviar_notificacion_gestion($id_gestion,$tipo,$usuario_dirigido,$descripcion) // envia notificacion de email 
{
 ...
 if (isset($entorno_desarrollo) ) {exit; }
    ...
    if (!mysqli_connect_errno()) {    
        ...
        if ($usuario_dirigido<>"") {
            ...
            $sql="select usuario.email from usuario
                LEFT OUTER JOIN usuario_nivelxgrupo ON (usuario.grupo_id=usuario_nivelxgrupo.grupo_id) 
                where usuario.activo='SI' 
                and usuario_nivelxgrupo.nivel_id=19
