...
ob_start(); // Importante para limpiar la salida antes del PDF
...
";
    ...
    // Validar si aplica promoción de octubre
    if (isset($row["aplica_promocion_octubre"]) && $row["aplica_promocion_octubre"] == 1 && $row["cuota_promocion_octubre"] > 0) {
        $cuota_final = $row["cuota_promocion_octubre"];
    } else {
        $cuota_final = $row["cuota"];
    }

    $html = str_replace("{cierre_cuota_cantidad}", $row["plazo"], $html);
    $html = str_replace("{cierre_cuota_total_usd}", formato_numero($cuota_final, 2, 'Lps. '), $html);
