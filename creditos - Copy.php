...
if ($accion=="2" or $accion=="25")  {
        ...
        //  if ($buscar=="") { echo mensaje( "Debe ingresar el numero de serie","warning"); exit;}
			...
			if (isset($_REQUEST['rfdesde'],$_REQUEST['rfhasta'])) {  
				...
				if (!checkfecha_mysql($fechadesde)) {$fechadesde= date('Y-m-01');}
				if (!checkfecha_mysql($fechahasta)) {$fechahasta= date('Y-m-t');}
			  } else {
    ...
    if (isset($_REQUEST['s'])) { // crear Solicitud nueva
     ...
     if ($_REQUEST["tipo_persona"]=="Persona Juridica" and $_REQUEST["nombre_empresa"]=="") {$verror.="Debe ingresar el nombre de la empresa"; }
        ...
        if ($verror==""){
            ...
            $sqlcampos.= " , tasa =".GetSQLValue(get_dato_sql('opciones','valor',' where id='.($_REQUEST["tipo_prima"] == 1 ? "1" : "3")) ,"text");
            $sqlcampos.= " , cierre_interes_mensual =".GetSQLValue(get_dato_sql('opciones','valor',' where id='.($_REQUEST["tipo_prima"] == 1 ? "2" : "4")) ,"text");
...
echo campo("tipo_prima","Tipo de Prima",'select',valores_combobox_texto('<option value="1">Prima Normal</option><option value="2">Prima Alta (40%)</option>',''),'class="form-control" "','','',3,5);
  ...
  echo '<div id="pjuridica" style="display: none">';
   ...
   echo '</div>';
     ...
     echo campo("identidad","Identidad",'text','','class="form-control" data-mask="9999-9999-99999?99" ','','',3,3);  
      ...
      echo campo("plazo","Plazo",'select',valores_combobox_texto('<option value="">Seleccione</option><option value="6">6</option><option value="12">12</option><option value="18">18</option><option value="24">24</option><option value="30">30</option><option value="36">36</option><option value="48">48</option>',''),'class="form-control" ','','',3,2);
...
<script> // Script para Guardar el documento/Solicitud
  function procesarforma() {
    $("#botones *").attr("disabled", "disabled");
    $("#solform :input").attr('readonly', true);
    $('#cargando').show();
    ...
    $.getJSON(url, $("#solform").serialize(), function(json) {
      i = 1;
      if (json.length > 0) {
        if (json[0].pcode == 0){ $('#salida').empty().append('<div class="alert alert-warning" role="alert">'+json[0].pmsg+'</div>'); }
        if (json[0].pcode == 1) {
          if (json[0].pcodid != 0) { $("#ridg").val(json[0].pcodid); }
          $("#datosgenerales *").attr("disabled", "disabled");
          $('#btnguardar').hide();
          //  $('#btnregresar').hide();
          $('#salida').empty().append('<div class="alert alert-success" role="alert">'+json[0].pmsg+'</div>'); 
...
<?php echo " </form></div></div></div></div>"; exit; } ?>
...
<script type="text/javascript">
  function selTipCred() {
    ...
    if(optSel=="2"){
      ...
      $('#plazo').append($('<option>', {value: 30}).text("30")); 
      $('#plazo').append($('<option>', {value: 36}).text("36")); 
    }
  }
</script>