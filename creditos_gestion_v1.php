...
if (isset($_REQUEST['cid'])) {
  $solicitud_id = $conn->real_escape_string($_REQUEST['cid']);
} else {
...
?>
  ...
  <br>
  <p><br><br><a href="#" onclick="actualizarbox('pagina','creditos.php') ; return false;" class="btn btn-default">REGRESAR</a></p>
  <?php
...
if ($accion == "2") //TODO Datos Generales
{
  ...
  if ($result->num_rows > 0) {
    ...
    <option value="Licencia de Conducir Vigente">Licencia de Conducir Vigente</option>', $row["tipo_identificacion"] ? $row["tipo_identificacion"] : "DNI"), 'class="form-control" "', '', '', 3, 5);
    ...
    ', $row["producto_servicio"] ? $row["producto_servicio"] : "Vehiculo Nuevo"), 'class="form-control" "', '', '', 3, 5);
    ...
    echo campo("moto_categoria", "Categoria", 'select', valores_combobox_texto('<option id="" value="motocicleta">Motocicleta</option><option id="" value="motocargo">Motocargo</option><option id="" value="mototaxi">Mototaxi</option><option id="" value="cuatrimoto">Cuatrimoto</option>', $row["moto_categoria"] ? $row["moto_categoria"] : "Motocicleta"), 'class=" form-control"', '', '', 3, 5);
    ...
    echo campo("monto_prestamo", "Valor Motocicleta", 'text', $row["monto_prestamo"], 'class="form-control" onchange="$(\'#monto_financiar\').val(convertir_num($(\'#monto_prestamo\').val())+convertir_num($(\'#monto_seguro\').val())+convertir_num($(\'#gastos_administrativos\').val())-convertir_num($(\'#monto_prima\').val())); " ', '', '', 3, 3);

    echo campo("monto_seguro", "Valor del Seguro", 'text', $row["monto_seguro"], 'class="form-control"  onchange="$(\'#monto_financiar\').val(convertir_num($(\'#monto_prestamo\').val())+convertir_num($(\'#monto_seguro\').val())+convertir_num($(\'#gastos_administrativos\').val())-convertir_num($(\'#monto_prima\').val())); " ', '', '', 3, 3);

    echo campo("monto_prima", "Prima", 'text', $row["monto_prima"], 'class="form-control"  onchange="$(\'#monto_financiar\').val(convertir_num($(\'#monto_prestamo\').val())+convertir_num($(\'#monto_seguro\').val())+convertir_num($(\'#gastos_administrativos\').val())-convertir_num($(\'#monto_prima\').val())); "', '', '', 3, 3);

    if (strpos($_SESSION['usuario'], 'Cd') === false || strpos($_SESSION['usuario'], 'Cd31') !== false) {
      echo campo("gastos_administrativos", "Gastos Administrativos", 'text', $row["gastos_administrativos"], 'class="form-control" onchange="$(\'#monto_financiar\').val(convertir_num($(\'#monto_prestamo\').val())+convertir_num($(\'#monto_seguro\').val())+convertir_num($(\'#gastos_administrativos\').val())-convertir_num($(\'#monto_prima\').val())); " ', '', '', 3, 3);
    } else {
      echo campo("gastos_administrativos", "Gastos Administrativos", 'text', $row["gastos_administrativos"], 'class="form-control" onchange="$(\'#monto_financiar\').val(convertir_num($(\'#monto_prestamo\').val())+convertir_num($(\'#monto_seguro\').val())+convertir_num($(\'#gastos_administrativos\').val())-convertir_num($(\'#monto_prima\').val())); " ', '', '', 3, 3, 'display: none;');
    }

    echo campo("monto_financiar", "Total Financiar", 'text', $row["monto_financiar"], 'class="form-control"  readonly', '', '', 3, 3);
    echo campo("plazo", "Plazo", 'select', valores_combobox_texto('<option value="">Seleccione</option><option value="6">6</option><option value="12">12</option><option value="18">18</option><option value="24">24</option><option value="36">36</option><option value="48">48</option>', $row["plazo"]), 'class="form-control" ', '', '', 3, 2);
...
if ($accion == "3") //TODO Datos Cliente o aval
{
  ...
  if ($result->num_rows > 0) {
    ...
    if ($row["tipo_persona"] == "Persona Juridica") {
      $pjestilo = '';
    } else {
      $pjestilo = 'style="display: none"';
    }
    ...
    echo campo("tipo_de_cliente", "Tipo de Cliente", 'select', valores_combobox_texto('<option value="">Seleccione</option>
<option value="PEP">PEP</option>
<option value="Publico no PEP">Publico no PEP</option>
...
<option value="APNFD">APNFD</option>
...
<option value="Otro">Otro</option>', $row["tipo_de_cliente"]), 'class="form-control" ', '', '', 3, 5);
...
<option value="Yoro">Yoro</option>', $row["departamento"]), 'class="form-control" ', '', '', 3, 3);
    ...
    echo campo("otra_nacionalidad", "¿Tiene otra nacionalidad?", 'select', valores_combobox_texto('
    <option value="SI">SI</option>
    <option value="NO">NO</option>
    ', $row["otra_nacionalidad"] ? $row["otra_nacionalidad"] : ""), 'class="form-control" onchange="seleccionNacionalidad();"', '', '', 3, 3);
    ...
    echo campo("empresa_propia", "¿La empresa es propia?", 'select', valores_combobox_texto('<option value="">Seleccione</option>
    <option value="SI">SI</option>
    <option value="NO">NO</option>', $row["empresa_propia"]), 'class="form-control" ', '', '', 3, 3);
    echo campo("empresa_direccion", "Direccion", 'text', $row["empresa_direccion"], 'class="form-control" ', '', '', 3, 9);
    echo campo("empresa_departamento", "Departamento", 'select', valores_combobox_texto('<option value="">Seleccione</option>
    <option value="Atlantida">Atlantida</option>
    <option value="Choluteca">Choluteca</option>
    <option value="Colon">Colon</option>
    <option value="Comayagua">Comayagua</option>
    ...
    <option value="Santa Barbara">Santa Barbara</option>
    <option value="Valle">Valle</option>
    <option value="Yoro">Yoro</option>', $row["empresa_departamento"]), 'class="form-control" ', '', '', 3, 4);
    ...
    echo campo("empresa_tipo_empleo", "Condicion Laboral", 'select', valores_combobox_texto('<option value="">Seleccione</option>
    <option value="ASALARIADO">ASALARIADO</option>
    <option value="INDEPENDIENTE">PROFESIONAL INDEPENDIENTE</option>
    <option value="COMERCIANTE/NEGOCIO PROPIO">COMERCIANTE/NEGOCIO PROPIO</option>
    <option value="OTRO">OTRO</option>', $row["empresa_tipo_empleo"]), 'class="form-control" ', '', '', 3, 3);
    ...
    echo campo("empresa_salario_tipo", "Por Concepto", 'select', valores_combobox_texto('<option value="">Seleccione</option>
    <option value="SALARIO">SALARIO</option>
    <option value="JUBILACION O PENSION">JUBILACIÓN O PENSIÓN</option>
    <option value="VENTAS BIENES MUEBLES/INMUEBLES">VENTAS BIENES MUEBLES/INMUEBLES</option>
    <option value="REMESAS">REMESAS</option>
    <option value="ALQUILERES">ALQUILERES</option>
    <option value="NEGOCIO PROPIO">NEGOCIO PROPIO</option>
    <option value="AHORROS">AHORROS</option>
    <option value="HERENCIA">HERENCIA</option>
    <option value="PREMIOS DE LOTERIA">PREMIOS DE LOTERÍA</option>
    <option value="PRESTAMO INTERNO">PRÉSTAMO INTERNO</option>
    <option value="PRESTAMO EXTERNO">PRÉSTAMO EXTERNO</option>
    <option value="OTROS">OTROS</option>', $row["empresa_salario_tipo"] ? $row["empresa_salario_tipo"] : 'SALARIO'), 'class="form-control" onchange="seleccionSalario();"', '', '', 3, 3);
    ...
    echo campo("origen_fondo_tercero", "¿El origen de los fondos fue proporcionado por un tercero?", 'select', valores_combobox_texto('<option value="">Seleccione</option>
    <option value="SI">SI</option>
    <option value="NO">NO</option>', $row["origen_fondo_tercero"]), 'class="form-control" onchange="origenTercero();"', '', '', 3, 3);
    ...
    echo campo("prevencion_lavado", "¿Es sujeto obligado según lo establecido en la normativa vigente en prevención de Lavado de Activos?", 'select', valores_combobox_texto('<option value="">Seleccione</option>
    <option value="SI">SI</option>
    <option value="NO">NO</option>', $row["prevencion_lavado"]), 'class="form-control" ', '', '', 3, 3);
    echo campo("acreditacion_ente", "¿Posee acreditación correspondiente, otorgada por el Ente Regulador?", 'select', valores_combobox_texto('<option value="">Seleccione</option>
    <option value="SI">SI</option>
    <option value="NO">NO</option>', $row["acreditacion_ente"]), 'class="form-control" ', '', '', 3, 3);
...
if ($accion == "5") //TODO gestiones
{
  ...
  if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
      ...
      echo '
     <li ' . $clase . ' onclick="actualizarSelect();"><a href="#b1" onclick=" actualizarbox(\'b1\',\'creditos_gestion.php?a=5b1&cst=' . $row["id"] . '&cid=' . $solicitud_id . '\') ; return false;" id="titulo' . $row["id"] . '" data-titulo="' . $row["nombre"] . '" class="text-muted">' . $icono . $row["nombre"] . $gpendientes . '</a></li>
  ...
  if ($verror == "") {
    ...
    if (isset($_POST)) {
      ...
      while (list($key, $val) = each($_POST)) {
        ...
        $val = stripslashes($val);
        if (
          $key <> "doc1" and $key <> "doc2" and $key <> "doc3" and
          $key <> "doc4" and $key <> "doc5" and $key <> "doc6" and
          $key <> "doc7" and $key <> "doc8" and $key <> "doc9" and
          $key <> "obs2_701" and $key <> "estado2_701" and $key <> "usr2_701"
        ) {
          $sqlcampos2 .= $sepa . $conn->real_escape_string($key) . "=" . GetSQLValue($conn->real_escape_string($val), "text");
          $sepa = " , ";
        }
...
// CUANDO CAMBIA EL TIPO DE PERFIL DEL CLIENTE AGREGA UN INTERÉS DISTINTO A LA SOLICITUD
if ($accion === "IntAdd") {
  $salida = "";
  $sql = "";
  $verror = "";
...
if ($accion == "5b1") //TODO gestiones
{
  ...
  if (isset($_REQUEST['s'])) {
    ...
    if ($_REQUEST['estado'] == 2 or $_REQUEST['estado'] == "2") {
      ...
      if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        if ($row["pendientes"] > 0) {
          $verror = 'No se puede aprobar la etapa porque hay ' . $row["pendientes"] . ' gestiones pendientes';
        }
      }
    }
  ...
  if (isset($_REQUEST['cst'])) {
    $cod_status = $conn->real_escape_string($_REQUEST['cst']);
  } else {
    exit;
  }

  $campo_unico = "";
  if (isset($_REQUEST['cpo'])) {
    $campo_unico = $conn->real_escape_string($_REQUEST['cpo']);
  }
  ...
  $mostrar_todo = false;
  if ($campo_unico == "701" and tiene_permiso(19)) {
    $mostrar_todo = true;
  }
    ...
    if ($result->num_rows > 0) {
      ...
      if ($campo_unico == "" or $campo_unico == 31 or $mostrar_todo) {
        ...
        if ($row["empresa_tipo_empleo"] == 'ASALARIADO') {
          ...
          <option value="PRESTAMO INTERNO">PRÉSTAMO INTERNO</option>
          <option value="PRESTAMO EXTERNO">PRÉSTAMO EXTERNO</option>
          <option value="OTROS">OTROS</option>', $row["empresa_salario_tipo"] ? $row["empresa_salario_tipo"] : 'SALARIO'), 'class="form-control"', '', '', 3, 3);
  ...
  if ($cod_status == "7") { //CALCULO FINANCIERO
    ...
    if ($result->num_rows > 0) {
      ...
      if ($campo_unico == ""  or $mostrar_todo) {
        ...
        // SELECT AGREGADO 
        echo campo("cierre_interes_mensual", "Perfil", 'select', valores_combobox_texto(
          '<option id="per_normal" value="3.98">Perfil Prima Normal</option>
            <option id="per_alta" value="3">Perfil Prima Alta</option>
            <option id="per_informal" value="4">Perfil Informal</option>
            <option id="per_ktm" value="3">Perfil KTM</option>
            <option id="per_mototaxi" value="4">Perfil Mototaxi</option>
            <option id="per_planilla" value="1.92">Convenio de Planilla</option>
            <option id="per_usados" value="4">Perfil Usados</option>',
          ($row["cierre_interes_mensual"] ? $row["cierre_interes_mensual"] : "3.98")
        ), 'class=" form-control" onchange="perfil_interes()"', '', '', 3, 3);

        // echo '<div id="div_perfil" class="" style="margin-left: 26%; margin-bottom: 1%; !important"><a id="interes" href="#" class="btn btn-primary" onclick="perfil_interes(' . $solicitud_id . ');"><span class="glyphicon glyphicon-floppy-open" aria-hidden="true"></span> Guardar Perfil del Cliente</a></div>';
  ...
  if ($cod_status == "8") { //CONDICIONES DE APROBACION
    ...
    if ($result->num_rows > 0) {
      ...
      if ($campo_unico == "") {
        ...
        echo campo("monto_financiar", "Total Financiar", 'text', $row["monto_financiar"], 'class=" form-control"  readonly', '', '', 3, 3);
        echo campo("plazo", "Plazo", 'select', valores_combobox_texto('<option value="">Seleccione</option><option value="6">6</option><option value="12">12</option><option value="18">18</option><option value="24">24</option><option value="30">30</option><option value="36">36</option>', $row["plazo"]), 'class=" form-control" onchange="calculos_financieros4(); "', '', '', 3, 2);
        echo campo("tasa", "Tasa", 'text', $row["tasa"], 'class=" form-control"  onchange="calculos_financieros4(); "', '', '', 3, 2); // AQUÍ EL OTRO POSIBLE
        echo '</div>';
