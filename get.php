...
if ($accion=="sub1") {
	 echo " <p>&nbsp;</p>";	
	 if (tiene_permiso(5)){  echo " <p><br><a href=\"#\" onclick=\"actualizarbox('pagina','get.php?a=11') ; return false;\" class=\"btn btn-lg btn-primary btn-block \">ACTIVAR GARANTIA</a></p>"; } 
	 if (tiene_permiso(4)){  echo " <p><br><a href=\"#\" onclick=\"actualizarbox('pagina','get.php?a=14') ; return false;\" class=\"btn btn-lg btn-primary btn-block \">CONSULTAR GARANTIAS</a></p>"; } 
	if (tiene_permiso(10)){  echo " <p><br><a href=\"#\" onclick=\"actualizarbox('pagina','get.php?a=18&b=p') ; return false;\" class=\"btn btn-lg btn-primary btn-block \">CONSULTAR PENDIENTES FACTURAR</a></p>"; } 
    if (tiene_permiso(11)){  echo " <p><br><a href=\"#\" onclick=\"actualizarbox('pagina','get.php?a=19&b=f') ; return false;\" class=\"btn btn-lg btn-primary btn-block \">CONSULTAR FACTURADAS</a></p>"; }
    if (tiene_permiso(3)){  echo " <p><br><a href=\"#\" onclick=\"actualizarbox('pagina','servicio.php') ; return false;\" class=\"btn btn-lg btn-primary btn-block \">MODULO DE SERVICIO</a></p>"; }	
...
if ($accion=="7") {
	$a2="";
		...
		if (isset($_REQUEST['o1'])) {$o1=$conn->real_escape_string($_REQUEST['o1']);}
		if (isset($_REQUEST['o2'])) {$o2=$conn->real_escape_string($_REQUEST['o2']); }
		if (isset($_REQUEST['o3'])) {$o3=$conn->real_escape_string($_REQUEST['o3']);}
		if (isset($_REQUEST['o4'])) {$o4=$conn->real_escape_string($_REQUEST['o4']);}
		if (isset($_REQUEST['a2'])) {$a2=$_REQUEST['a2'];}
		...
		if ($a2=="u") {
			$verror="";
			if ($o2=="" or $o3=="") {echo "Debe ingresar la contrase&ntilde;a actual y la nueva"; exit;}
			$verror=validar("Contrase&ntilde;a Nueva",$o3, "text", true,null, 6);
			if ($verror<>"") {echo $verror; exit;}
			
			$cactual=get_dato_sql("usuario","clave"," where id=".$_SESSION['usuario_id']);
			if (!verificar_password($o2,$cactual)) {echo "La contrase&ntilde;a actual no es correcta"; exit;}
			
			$sqlu="update usuario set clave='".generate_hash($o3)."' where id=".$_SESSION['usuario_id'];
			
			if ($conn->query($sqlu) === TRUE) 
			{	echo "ok";	} else { echo "Error al actualizar contrase&ntilde;a";}
		...
		<div class="control-group"><label class="control-label">Repita la Contrase&ntilde;a Nueva:</label><div class="controls"> <input name="c22" id="mccb2" type="password" class="" ></div></div>
		</div>
		';
		...
		if ($a2=="u") {
			$verror="";
			if ($o4=="" or $o3=="") {echo "Debe ingresar la contrase&ntilde;a nueva"; exit;}
			$verror=validar("Contrase&ntilde;a Nueva",$o3, "text", true,null, 6);
			if ($verror<>"") {echo $verror; exit;}
...
if ($accion=="8") {
				if (!tiene_permiso(1)) {exit; }
				
				if (isset($_REQUEST['o'])) { $opcion = $_REQUEST['o']; } else exit;
				if (isset($_REQUEST['rid'])) { $r_id = $_REQUEST['rid']; } else exit;
			//*** guardar  ****
				if ($opcion=="sv") {
					$salida= "";
					$sql_errores=false;
					...
					// borrar registros actuales
					 if ($r_id<>0 and $r_id<>"") {if ($conn->query("delete from usuario_nivelxgrupo where grupo_id=$r_id") === FALSE) {$sql_errores=true; } }
					
				
					// guardar registros
					if (count($losdatos)>0) {
						foreach($losdatos as $value){
						  if ($conn->query("insert into usuario_nivelxgrupo set grupo_id=$r_id , nivel_id=$value") === FALSE) {$sql_errores=true; break;}
						}
					} 
					
					if ($sql_errores == true) {$conn -> rollback();	} else {$conn -> commit(); $salida="ok";	}
					    ...
					    <table align="center" width="80%">
					            <tr>
					                <td>
				                        ...
				                        <select id="box1View" multiple="multiple" style="height:350px;width:370px;">
					                ...
					                <td>
					                    ...
					                    <select id="box2View"  multiple="multiple" style="height:350px;width:370px;">
											
											<?php if ($r_id<>0 and $r_id<>"") {echo valores_combobox_db_permisos_asignados($r_id); }?>
					                    </select><br/>
...
if ($accion=="81" or $accion=="82") {
				...
				if ($accion=="81" ) { $tabla="distribuidor"; } else  { $tabla="bodega"; } 
			...
			//*** guardar  ****
				if ($opcion=="sv") {
					...
					// borrar registros actuales
					 if ($r_id<>0 and $r_id<>"") {if ($conn->query("delete from ".$tabla."xusuario_grupo where grupo_id=$r_id") === FALSE) {$sql_errores=true; } }
					
				
					// guardar registros
					if (count($losdatos)>0) {
						foreach($losdatos as $value){
						  if ($conn->query("insert into ".$tabla."xusuario_grupo set grupo_id=$r_id , ".$tabla."_id='$value'") === FALSE) {$sql_errores=true; break;}
						}
					} 
					            ...
					            <tr>
					                ...
					                <td>
					                    ...
					                    <select id="box2View"  multiple="multiple" style="height:350px;width:300px;">
											...
											<?php if ($r_id<>0 and $r_id<>"") {echo valores_combobox_db_distribuidores_bodegas_asignados($r_id,$tabla); }?>
					                    </select><br/>
					                    <span id="box2Counter" class="countLabel"></span><select id="box2Storage"></select>
					                </td>
					            </tr>
...
if ($accion=="11") { // TODO  pagina ver garantia
	if (!tiene_permiso(2)) { echo mensaje("No tiene privilegios para accesar esta seccion","danger");exit;}
...
<div id="buscar"> 
	...
	if (isset($_REQUEST['sub']) ) 
	{
		...
		//	if ($buscar=="") { echo mensaje( "Debe ingresar el numero de serie","warning"); exit;}
			// get.php?a=11&sub=1&sb=&creditos=1
			//******* SQL ************************************************************************************
			//SAP
				$sql=  
				 "SELECT distinct T1.ItemCode ,T1.ItemName,T0.SuppSerial SerieChasis,
				    (select top 1 DistNumber from OSRN  with (nolock) where MnfSerial=T0.SuppSerial)as SerieMotor,
			        (select top 1 LotNumber from OSRN  with (nolock) where MnfSerial=T0.SuppSerial)as anio,
				  T0.WhsCode Almacen,T5.Name Marca,T6.Name Modelo,t4.Name Color,T7.Name ModeloESpecifico,T8.U_CardCode CodigoCliente
				  ,T8.WhsName NombreAlmacen
				  FROM OSRI T0  with (nolock)
			      INNER JOIN OITM T1  with (nolock) ON T0.ItemCode = T1.ItemCode 
			      INNER JOIN OITB T2  with (nolock) ON T1.ItmsGrpCod =     T2.ItmsGrpCod
			      inner join [@SCOLOR] t4  with (nolock) on T1.U_ACOLOR = t4.Code
			      inner join [@AMARCA] T5  with (nolock) ON T1.U_AMARCA = T5.Code
			      INNER JOIN [@AMODELO] T6  with (nolock) ON T1.U_AMODELO=T6.Code
			      INNER JOIN [@OITMMODELOSMOTOS] T7   with (nolock)ON T1.U_ModelID=T7.Code
			      Inner join OWHS T8  with (nolock) on T0.WhsCode = T8.WhsCode
				  WHERE   T2.[ItmsGrpNam] ='Motocicleta'  and t0.[status] = '0'"; //and (select top 1 U_StatusSerie from OSRN where MnfSerial=T0.SuppSerial)<>'02' 
			...
			//if (!tiene_permiso(12)) {$sql.=" and T0.[Status]  = '0'";}
			//if (isset($_REQUEST['creditos'])) {$sql.=" and T0.[Status]  = '0'"; }
			
			//DISTRIBUIDOR: select CardCode,CardName from OCRD
			//BODEGAS: select WhsCode,WhsName from OWHS
			
			$campo_dist="T8.U_CardCode";
            ...
            if (isset($entorno_desarrollo) ) {
					...
					if (!tiene_permiso(12)) {$sql.=" and [Status]  = '0'";}
                    if (isset($_REQUEST['creditos'])) {$sql.=" and [Status]  = '0'"; }
	...
	//<table class="table table-striped">
			...
			while( $row = sqlsrv_fetch_array( $stmt2, SQLSRV_FETCH_ASSOC) )
                ...
                $regvalido = $conn -> query("select id from garantia where nulo='NO' and serie_motor='".$row["SerieMotor"]."'");
                if ($regvalido -> num_rows <= 0) 
                { 
                    
				echo "<tr>";
                
                if (isset($_REQUEST['creditos'])) {            
                                                                                                     
                  echo " <td class='text-center'><a  href=\"#\" class=\"btn btn-default btn-sm\" onclick=\"$('#moto_serie').val('".trim($row["SerieChasis"])."'); $('#moto_marca').val('".trim($row["Marca"])."');$('#moto_modelo').val('".trim($row["Modelo"])."');$('#moto_motor').val('".trim($row["SerieMotor"])."');$('#moto_color').val('".trim($row["Color"])."');$('#moto_ano').val('".trim($row["anio"])."');$('#moto_cilindraje').val('".trim($row["ModeloESpecifico"])."'); $('#motospanel').html(''); return false;\" ><span class=\"glyphicon glyphicon-folder-open\" aria-hidden=\"true\"></span></a></td>";
                  
                    
                } else {
		         echo " <td class='text-center'><a  href=\"#\" class=\"btn btn-default btn-sm\" onclick=\"actualizarbox('pagina','get.php?a=12&cid=".trim($row["SerieChasis"])."') ; return false;\" ><span class=\"glyphicon glyphicon-folder-open\" aria-hidden=\"true\"></span></a></td>";
		         }

                 echo " <td class='text-center'>".trim($row["Marca"])."</td>
		              <td class='text-center'>".trim($row["Modelo"])."</td> ";
		         
		         if (isset($_REQUEST['creditos'])) {
		          echo "  <td >".trim($row["SerieChasis"])."</td>";   
                 } else { 
		         echo "  <td ><a  href=\"#\" onclick=\"actualizarbox('pagina','get.php?a=12&cid=".trim($row["SerieChasis"])."') ; return false;\" >".trim($row["SerieChasis"])."</a></td>";
		        }
		        echo "  <td class='text-center'>".trim($row["SerieMotor"])."</td>
		              ...
		              <td class='text-center'>".trim($row["Color"])."</td>
		               ...
		               <td class='text-center '>".trim($row["NombreAlmacen"])."</td>
		              </tr>"	;
				$reg++;
				}
...
if ($accion=="12") { // TODO  pagina Mant serie
	if (!tiene_permiso(2)) { echo mensaje("No tiene privilegios para accesar esta seccion","danger");exit;}
	 ...
	 if (!isset($_REQUEST['cid'])) {echo mensaje( "Debe seleccionar un registro","warning"); exit;}
	 $codigo=$conn->real_escape_string($_REQUEST['cid']);
	 if ($codigo=="") {echo mensaje( "Debe seleccionar un registro","warning"); exit;}
...
//******* SQL ************************************************************************************
                ...
                (select top 1 LotNumber from OSRN where MnfSerial=T0.SuppSerial)as anio,
                      ...
                      WHERE   T2.[ItmsGrpNam] ='Motocicleta' 	and  T0.SuppSerial = '$codigo'						
	 		  ";
			// if ($_SESSION['usuario_distribuidor']<>"") {$sql.=" and T8.U_CardCode='".$_SESSION['usuario_distribuidor']."'";} 
...
if (!tiene_permiso(12)) {$sql.=" and T0.[Status]  = '0'";}
		...
		if (sqlsrv_has_rows($stmt2)==false) { echo mensaje( "No se encontraron registros","warning");exit;}
  ...
  <tr>
     ...
     <th>Almacen</th>
	  ...
	  // echo campotabla("Descripcion","Descripcion",$texto_seg,$row["Descripcion"],'class="form-control" ');	
	   ...
	   echo campotabla_columna("Almacen","Almacen",$texto_seg,$row["NombreAlmacen"],'class="form-control" ');
	...
	<?php
	 ...
	 if (tiene_permiso(5)) {
		  ...
		  // if ( (trim($row["Almacen"])==trim($_SESSION['usuario_bodega'])) or tiene_permiso(9)) 
		   {
		?>
	<a href="#" class="btn btn-primary" onclick="actualizarbox('activar_garantia','get.php?a=13&cid=<?php echo urldecode($row["SerieChasis"]); ?>') ; $(this).hide(); return false;"><span class="glyphicon glyphicon-certificate" aria-hidden="true"></span> Activar Garant&iacute;a</a>	
	<?php } } ?>
	<form class="form-horizontal" id="garantiaform">
       ...
       echo campo("U_Rodaje","",'hidden',$row["U_Rodaje"],'');
...
if ($accion=="13") { // TODO  Activar serie
	if (!tiene_permiso(5)) { echo mensaje("No tiene privilegios para accesar esta seccion","danger");exit;}
	...
	if (isset($_REQUEST['s'])) {
		...
		// $verror.=validar("Numero de Factura",$_REQUEST['factura_numero'], "text", true,  null,  null,  null);
         ...
         $verror.=validar("Tipo de Venta",$_REQUEST['tipo_venta'], "text", true,  null,  3,  null);
		...
		$verror.=validar("Direccion",$_REQUEST['direccion'], "text", true,  null,  3,  null);
		$verror.=validar("Ciudad",$_REQUEST['ciudad'], "text", true,  null,  3,  null);
		$verror.=validar("Departamento",$_REQUEST['departamento'], "text", true,  null,  3,  null);
		...
		if ($verror==""){
			...
			$rodaje="2";
			if (isset($_REQUEST["U_Rodaje"])) {
				if ($_REQUEST["U_Rodaje"]=='02') {$rodaje="4";	}
			}
			$sqlcampos.= " , rodaje =".GetSQLValue($rodaje,"text");
            ...
            $sqlcampos.= " , tipo_venta =".GetSQLValue($conn->real_escape_string($_REQUEST["tipo_venta"]),"text");
            $sqlcampos.= " , nombre_vendedor =".GetSQLValue($conn->real_escape_string($_REQUEST["nombre_vendedor"]),"text");
            $sqlcampos.= " , email =".GetSQLValue($conn->real_escape_string($_REQUEST["factura_numero"]),"text");
			...
			if (isset($_REQUEST["doc4"])) {$sqlcampos.= " , doc4 =".GetSQLValue($conn->real_escape_string($_REQUEST["doc4"]),"text"); }
			if (isset($_REQUEST["doc5"])) {$sqlcampos.= " , doc5 =".GetSQLValue($conn->real_escape_string($_REQUEST["doc5"]),"text"); }
			if (isset($_REQUEST["doc6"])) {$sqlcampos.= " , doc6 =".GetSQLValue($conn->real_escape_string($_REQUEST["doc6"]),"text"); }
			
			$sqlcampos.= ",usuario_alta= '" .$_SESSION['usuario'] . "' ,fecha_alta=now()";
			
			
           $cod_bodega=$conn->real_escape_string($_REQUEST["Almacen"]); 
           ...
           $bodega_nombre= get_dato_sql('bodega', 'nombre', " where codigo='$cod_bodega'");
            ...
            SAP_actualizar_estado_moto($conn->real_escape_string($_REQUEST["SerieChasis"]));
	...
	?> 
       ...
       echo campo("nombre_vendedor","Nombre Vendedor",'text','','class="form-control" ','','',3,7);
       
     echo campo("factura_numero","No. Factura",'text','','class="form-control" ','','',3,3);
      echo campo("tipo_venta","Tipo Venta",'select',valores_combobox_texto('<option value="">Seleccione</option><option value="Contado">CONTADO</option><option value="Credito">CREDITO</option>',''),'class="form-control" ','','',3,4);
     
	   echo campo("nombres","Nombres",'text','','class="form-control" ','','',3,7);
	   echo campo("apellidos","Apellidos",'text','','class="form-control" ','','',3,7);
	   echo campo("identidad","No. de Cedula",'text','','class="form-control" data-mask="9999-9999-99999?99"','','',3,4);
...
<option value="Yoro">Yoro</option>',''),'class="form-control" ','','',3,4);
	   ...
	   echo campo("telefono","Telefono Casa",'text','','class="form-control" data-mask="9999-9999"','','',3,4);
	   echo campo("celular","Celular",'text','','class="form-control" data-mask="9999-9999"','','',3,4);
        echo campo("email","Email",'text','','class="form-control" ','','',3,7);
...
if ($accion=="14") { // TODO  consultar garantias activadas
	if (!tiene_permiso(4)) { echo mensaje("No tiene privilegios para accesar esta seccion","danger");exit;}
...
<div id="buscar"> 
	...
	if (isset($_REQUEST['sub']) ) 
	{
		...
		//	if ($buscar=="") { echo mensaje( "Debe ingresar el numero de serie","warning"); exit;}
			...
			,tipo_venta,nombre_vendedor,email,comentario
			, (DATEDIFF(CURDATE(),fecha_alta) ) as antiguedad 
			FROM garantia 
			WHERE  1=1 ";
		
        
			if ($buscar<>"") { $sql.=  "  and ( serie_chasis LIKE '%$buscar%' )";	
			}		
           ...
           if ($_SESSION['usuario_bodega']<>"") {$sql.=" and  bodega='".$_SESSION['usuario_bodega']."'";} 
            if ($_SESSION['usuario_distribuidor']<>"") {$sql.=" and distribuidor='".$_SESSION['usuario_distribuidor']."'";}     
            }
                      ...
                      <table class="display nowrap" id="tabla" width="100%" cellspacing="0">
                        ...
                        while ($row = $result -> fetch_assoc()) {
                           echo "<tr>
                              ...
                              <td ><a  href=\"#\" onclick=\"actualizarbox('pagina','get.php?a=15&cid=".trim($row["serie_motor"])."&eid=".$row["id"]."') ; return false;\" >".$row["serie_chasis"]."</a></td>
                               <td class='text-center'>".$row["serie_motor"]."</td>
...
if ($accion=="15") { // TODO  serie  - subir doctos o anular
    if (!tiene_permiso(2)) { echo mensaje("No tiene privilegios para accesar esta seccion","danger");exit;}
     ...
     if ($codigo=="") {echo mensaje( "Debe seleccionar un registro","warning"); exit;}
            ...
            if ($result -> num_rows <= 0) { echo mensaje( "No se encontraron registros","warning");exit;}
  ...
  <tr>
    ...
    <div class="panel panel-default">
      ...
      echo campo("nombre_vendedor","Nombre Vendedor",'text',$row["nombre_vendedor"],'class="form-control" readonly','','',3,7);
      
       echo campo("factura_numero","No. Factura",'text',$row["factura_numero"],'class="form-control" readonly ','','',3,3);    
       echo campo("tipo_venta","Tipo Venta",'text',$row["tipo_venta"],'class="form-control" readonly','','',3,4);
       ...
       echo campo("email","Email",'text',$row["email"],'class="form-control" readonly ','','',3,7);
       echo campo("sexo","Sexo",'text',$row["sexo"] ,'class="form-control" readonly ','','',3,3);
        echo campo("comentario","Comentario",'textarea',$row["comentario"] ,'class="form-control" readonly ','','',3,7);
  ...
  <a id="btnimprimirgarantia" href="#"  class="btn btn-info" onclick="imprimir_garantia(<?php echo $row["id"]; ?>) ; return false;"><span class="glyphicon glyphicon-print" aria-hidden="true"></span> Imprimir Garantia</a>
    ...
    <?php
     ...
     if ($row["nulo"]=="SI" ) {
        ?>
     <script>$('#btnimprimir').show();</script>

<?php } else {
         if (tiene_permiso(8)) {  ?>
             &nbsp;&nbsp;&nbsp;&nbsp;
        <a id="btnanular" href="#" class="btn btn-danger" onclick="actualizarbox_confirmar('accion','get.php?a=16&cid=<?php echo trim($row["serie_chasis"])."&eid=".$row["id"]; ?>','Anular','Desea anular la garantia?','ui-icon-comment','btnanular') ;  return false;"><span class="glyphicon glyphicon-ban-circle" aria-hidden="true"></span> Anular Garant&iacute;a</a>    
...
if ($accion=="16") { // TODO   anular garantia
    if (!tiene_permiso(8)) { echo mensaje("No tiene privilegios para accesar esta seccion","danger");exit;}
    
     if (!isset($_REQUEST['cid'],$_REQUEST['eid'])) {echo mensaje( "Debe seleccionar un registro","warning"); exit;}
     $codigo=$conn->real_escape_string($_REQUEST['cid']);
     $eid=$conn->real_escape_string($_REQUEST['eid']);
     if ($codigo=="") {echo mensaje( "Debe seleccionar un registro","warning"); exit;}
            ...
            $sql=  "SELECT id   
                    nulo ,  nulo_fecha ,  nulo_usuario ,  nulo_numero ,  nulo_motivo 
                    FROM garantia 
                    WHERE    serie_chasis = '$codigo' and id=$eid                         
            "; 
            ...
            if (SAP_verificar_estado_moto($codigo)<>0) {echo mensaje( "No puede anular la garantia, porque debe estar el estado en disponible en el sistema SAP","warning");exit; }
            
             $row = $result -> fetch_assoc();
            
            if ($row['nulo']=='SI') {echo mensaje( "La garantia ya se encuentra Nula","warning");exit;}
           ...
           // $sqlcampos.= " , nulo_motivo =".GetSQLValue($conn->real_escape_string($_REQUEST["nulo_motivo"]),"text");
            ...
            $sql="update garantia set nulo='SI' ,factura_numero_movesa=null " . $sqlcampos ." where  serie_chasis = '$codigo' and id=$eid ";
            ...
            } else { echo mensaje( "Error al Anular garantia","warning");exit;}
...
if ($accion=="17") { // TODO   subir docto despues de creada garantia
    if (!tiene_permiso(5)) { echo mensaje("No tiene privilegios para accesar esta seccion","danger");exit;}
     ...
     if (!isset($_REQUEST['cid'],$_REQUEST['eid'],$_REQUEST['dd'])) {echo mensaje( "Debe seleccionar un registro","warning"); exit;}
     $codigo=$conn->real_escape_string($_REQUEST['cid']);
     $eid=$conn->real_escape_string($_REQUEST['eid']);
...
if ($accion=="18") { // TODO  consultar garantias pendientes facturar 
    if (!tiene_permiso(10)) { echo mensaje("No tiene privilegios para accesar esta seccion","danger");exit;}
            
            if (!isset($_REQUEST['b'])) {echo mensaje( "Debe seleccionar un registro","warning"); exit;}
...
if ($accion=="19") { // TODO  consultar garantias facturadas 
    if (!tiene_permiso(10)) { echo mensaje("No tiene privilegios para accesar esta seccion","danger");exit;}
  ...
  //Filtro de fecha
        ...
        <div id="buscar"> 
            <div class="panel panel-default">
                <div class="panel-heading">
                  <h4 class="panel-title">Garantias Facturadas</h4>
                </div>
                <div class="panel-body">
             ...
             <form class="form-horizontal">
               <?php 
                ...
                echo campo("fd","Fecha Desde:","date","",'class="form-control" autofocus','','',3,3);
                echo campo("fh","Fecha Hasta:","date","",'class="form-control" ','','',3,3); 
        
               echo campo( "Aceptar","Buscar","botonlink","","onclick=\"procesarrep2() ; return false;\" ");             
               ?> 
                ...
                <div id="reportev2"> </div>
                
                 <script type="text/javascript">
                    function procesarrep2() {
                        
                        var url = "get.php?a=19&b=0&sub=1&fd="+$("#fd").val()+"&fh="+$("#fh").val() ;             
                        actualizarbox('reportev2',url) ; 
  ...
  //Mostrar consulta  
    ...
    if (isset($_REQUEST['sub']) ) 
    {
        ...
        if ($fechaerror<>"") { echo mensaje($fechaerror,'danger'); exit;}

        $filtro_fecha=" and factura_fecha_movesa>='$fdesde' and factura_fecha_movesa<='$fhasta'";
            ...
            marca, estilo, modelo, color, serie_motor, serie_chasis,anio, 
            nulo, nulo_fecha, nulo_usuario, nulo_numero, nulo_motivo, doc1, doc2, doc3, doc4, doc5, doc6
            , (DATEDIFF(CURDATE(),fecha_alta) ) as antiguedad ,factura_numero_movesa,factura_fecha_movesa
            FROM garantia 
            WHERE  1=1 ";
            
             $sql.=" $filtro_fecha ";
            $sql.=" and factura_numero_movesa is not null ";
           ...
           if ($_SESSION['usuario_bodega']<>"") {$sql.=" and  bodega='".$_SESSION['usuario_bodega']."'";} 
            ...
            // ****** Fin SQL ********************************************************************************
             ...
             $result = $conn -> query($sql);
              ...
              echo "<h4>$titulo del $reporte_fecha</h4><br> ";
            if ($result -> num_rows > 0) {
                        ...
                        while ($row = $result -> fetch_assoc()) {
                           echo "<tr>
                              ...
                              <td class='text-center'>".$row["color"]."</td>
                               ...
                               echo "<td class='text-center'>".$row["factura_numero_movesa"]."</td>";
                               echo "<td class='text-center'>".fechademysql($row["factura_fecha_movesa"])."</td>";
